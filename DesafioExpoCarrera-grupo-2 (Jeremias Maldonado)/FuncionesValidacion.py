...
def validar_usuario_instagram(usuario):
    # Expresión regular para validar el formato del usuario de Instagram
    patron = r'^[a-zA-Z0-9._-]+$'
    # Comprobar si el usuario cumple con el patrón
    if not re.match(patron, usuario.lower()):
        messagebox.showerror("Error", f"{usuario} : no es un usuario de Instagram válido. Solo se permiten números, letras, puntos y guiones bajos.")
        return False
    return True
